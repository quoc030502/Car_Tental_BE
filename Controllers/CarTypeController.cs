...
namespace basic_api.Controllers
{
  ...
  public class AdminCarTypeController(ICarTypeInterface carTypeRepo) : ControllerBase
  {
    ...
    public async Task<IActionResult> Delete([FromRoute] int id)
    {
      ...
      if (carType == null) return NotFound(ErrorMessages.CarNotFound);

      if (carType.Cars == null || carType.Cars.Any()) return BadRequest(ErrorMessages.CarTypeIsHavingCar);

      await _carTypeRepo.Delete(id);
      return Ok();
    }
